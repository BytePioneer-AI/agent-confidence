...
(function(){
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    /* ===== Mouse navigation: wheel + click =====
     * Wheel down → next
     * Wheel up   → prev
     * Skips interactive targets, overview, and notes drawer.
     */
    function isUiChrome(target) {
      if (!target || !target.closest) return false;
      return !!target.closest(
        'a, button, input, textarea, select, summary, ' +
        '.overview, .notes-overlay, .notes.open, aside.notes, ' +
        '.progress-bar, .theme-indicator, .anim-indicator'
      );
    }
    function isOverlayOpen() {
      const ov = document.querySelector('.overview');
      const no = document.querySelector('.notes-overlay');
      return (ov && ov.classList.contains('open')) ||
             (no && no.classList.contains('open'));
    }

    let wheelLock = false;
    window.addEventListener('wheel', function (e) {
      if (isOverlayOpen() || isUiChrome(e.target)) return;
